...
public class ReadJSONObjectAsFlattenedPropertiesMojo extends AbstractMojo {
  ...
  private static abstract class TFStateResource {
  ...
  private static class ArtifactRepoResource extends TFStateResource {
    ...
    public ArtifactRepoResource(ArtifactRepoRemoteState rs, ArtifactRepoRemoteState defaults, Settings settings)
  ...
  private static class S3UrlResource extends TFStateResource {
    ...
    public S3UrlResource(S3RemoteState rs, S3RemoteState defaults, Settings settings) throws MojoExecutionException {
      this.rs = Objects.requireNonNull(rs, "s3 remote state must not be null").validate(defaults, settings);
    }
